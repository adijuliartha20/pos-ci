...
class Districts extends CI_Controller {
     ...
     public function index($slug='publish'){
        if(isset($_POST['action-table']) && !empty($_POST['action-table']) ){
            if ($_POST['action-table']=='trash' || $_POST['action-table']=='untrash'){
                $this->districts_model->update_state();    
            } else if($_POST['action-table']=='delete'){
                $this->delete();
            }
        }

        $data['data'] = $this->districts_model->get_data($slug);
        //print_r($data['data']);
    ...
    public function create($id=''){
        ...
        if(!empty($id)){
            $data['data'] = $this->districts_model->get_single_data($id);
            $data['mode'] = 'edit';
        ...
        $data['province'] =  $this->province_model->get_data('publish',1);

        //print_r($data['province']);


        add_footer_js('admin-districts.js');
        $this->load->view('admin/templates/header', $data);
        $this->load->view('admin/templates/sidebar', $data);
    ...
    public function save(){
        ...
        if($mode=='edit') {
            $config =   array(
                            array('field' => 'name_district','label' => 'Kabupaten','rules' => 'trim|required', 'errors' => array('required' => 'Kabupaten harus diisi'))
                        );
        }else{
            $config =   array(
                            array('field' => 'name_district','label' => 'Kabupaten','rules' => 'trim|required|is_unique[districts.name_district]',
                                  'errors' => array('required' => 'Kabupaten harus diisi', 'is_unique' => 'Nama Kabupaten sudah ada')
                                ),
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            if($mode=='edit'){//update
                $this->districts_model->update($this->input->post('id_district'));
                $return['status'] = 'success';
                $return['msg'] = '<p><strong>Berhasil!!!</strong> '.strtolower($this->title).'</p>';
            }else{
                $this->districts_model->add_new();  
                $return['status'] = 'success';
    ...
    public function change_status(){
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            if($action=='delete'){
                ...
                if($this->districts_model->update_state_single($id, $action)){
    ...
    public function get_list_district(){
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $districts = $this->districts_model->get_data('publish',$id_province);


            //print_r($districts);
            $dts =  array();
            foreach ($districts as $key => $value) {
                $dts[$key]['id'] = $value['id_district'];
                $dts[$key]['name'] = $value['name_district'];
            }
            $return['districts'] = $dts;
            $return['status'] = 'success';
        }
