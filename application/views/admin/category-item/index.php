<div class="content-i">
    <div class="content-box">
      	<div class="element-wrapper">
	        <h6 class="element-header">Data Kategori Barang<input type="button" name="" value="Tambah" class="btn btn-primary"  onclick="location.href = '<?php echo ADMIN_URL.'/'.$app.'/create'; ?>';"></h6>
				    		...
				    		<tbody>
				    			<?php 
				    				foreach ($data as $key => $dt) {
				    					...
				    					?>
				    				<tr id="tr-<?php echo $dt['id_cat']; ?>">
				    					<td>
				    						<input type="checkbox" name="ids[]" value="<?php echo $dt['id_cat']; ?>" >
				    					</td>
				    					<td>
				    						<p class="text-action">
				    							<label>
				    								<span><?php echo $dt['category'];?></span>	
				    							</label>		    							
				    						</p>
				    						<p class="small-action">
				    							<?php 
				    								if($state=='trash'){?>
				    									<a onclick="action_row(event,<?php echo $dt['id_cat']; ?>,'publish')">Restore</a> | <a onclick="action_row(event,<?php echo $dt['id_cat']; ?>,'delete')">Delete Permanen</a>
				    								<?php
				    								}else{?>
				    									<a href="<?php echo ADMIN_URL.'/'.$app.'/edit/'.$dt['id_cat']; ?>">Edit</a> | <a onclick="action_row(event,<?php echo $dt['id_cat']; ?> , 'trash')">Pindah ke Sampah</a>
				    								<?php
				    								}
				    							?>
